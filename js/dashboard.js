...
function renderBoards() {
    ...
    starBoardSection.innerHTML = ``;
    for (let i = 0; i < data.users[currentUser].boards.length; i++) {
        let board = data.users[currentUser].boards[i];
        if (board.is_starred) {
            let boardItemStar = document.createElement("div");
        boardItemStar.classList.add("col-6", "col-md-6", "col-lg-3");
        boardItemStar.innerHTML = `
                <div class="board-card" style="background-image: url('${board.backdrop}');">
            <div class="board-title">${board.title}</div>
            <div class="btn btn-primary editon" data-bs-toggle="modal" data-bs-target="#editboard" onclick="editBoard(${i})">
    <img src="/assets/icons/edit-icon.png" alt="">
    <span>chỉnh sửa board</span>
  </div>
        <div class="btn btn-primary deleteon" data-bs-toggle="modal" data-bs-target="#deleteboard" onclick="editBoard(${i})">X</div>            
          </div>
          </div>
        `;
        starBoardSection.appendChild(boardItemStar);
            continue; 
        }
...
function editBoard(index) {
    document.getElementById("editboardTitle").value = data.users[currentUser].boards[index].title;
    indexedit=index
...
document.getElementById("formreate").addEventListener("submit", function (event) {
    ...
    let newBoard = {
        id: data.users[currentUser].boards.length + 1,
    ...
    data.users[currentUser].boards.push(newBoard);
    localStorage.setItem("users", JSON.stringify(data));
    boardSection.innerHTML = ``;
    renderBoards()
    boardTitle.value = "";
...
document.getElementById("deleteboard").addEventListener("submit", function (event) {
    ...
    if (boardIndex < 0 || boardIndex >= data.users[currentUser].boards.length) {
        alert("Số thứ tự không hợp lệ!");
        return;
    }
    data.users[currentUser].boards.splice(boardIndex, 1);
    localStorage.setItem("users", JSON.stringify(data));
    renderBoards()
})
...
document.getElementById("formeditboard").addEventListener("submit", function (event) {
    ...
    data.users[currentUser].boards[boardIndex].title = newTitle;
    data.users[currentUser].boards[boardIndex].backdrop = newBackdrop;
    localStorage.setItem("users", JSON.stringify(data));
    renderBoards()
    newTitle.value = "";
    document.querySelector('input[name="color"]:checked').checked = false;
